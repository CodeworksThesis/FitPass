...
export default function Search() {
    ...
    const handleTime = (newButton: string) => {
        timeButtons.includes(newButton)
            ? setTimeButtons((oldButtons) =>
                  oldButtons.filter((el) => {
                      return el !== newButton
                  })
              )
            : setTimeButtons([...timeButtons, newButton])
    }

    const handleCategory = (newButton: string) => {
        categoryButtons.includes(newButton)
            ? setCategoryButtons((oldButtons) =>
                  oldButtons.filter((el) => {
                      return el !== newButton
                  })
              )
            : setCategoryButtons([...categoryButtons, newButton])
    }
        ...
        <main className="mt-20">
            ...
            <section className="ml-8 font-black mt-6">
                ...
                <form>
                    ...
                    <div className="relative">
                        ...
                        <input
                            type="search"
                            id="location-search"
                            className="z-0 block p-4 pl-10 w-full text-sm text-gray-900 bg-white rounded-lg border border-gray-300 placeholder-gray-400 pl-[55px] w-[90%] drop-shadow-md"
                ...
                <section className="mt-6">
                    ...
                    <ButtonSearch
                        buttonText="Morning"
                        buttonClick={() => handleTime("Morning")}
                        isPressed={timeButtons.includes("Morning")}
                    />
                    <ButtonSearch
                        buttonText="Afternoon"
                        buttonClick={() => handleTime("Afternoon")}
                        isPressed={timeButtons.includes("Afternoon")}
                    />
                    <ButtonSearch
                        buttonText="Evening"
                        buttonClick={() => handleTime("Evening")}
                        isPressed={timeButtons.includes("Evening")}
                    />
                </section>
                <section className="mt-6">
                    <h2>Price</h2>
                ...
                <section className="mt-6">
                    ...
                    {categories.map((category, index) => (
                        <ButtonSearchCategories
                            key={index}
                            buttonText={category}
                            buttonClick={() => handleCategory(category)}
                            isPressed={categoryButtons.includes(category)}
                        />
                    ))}
                </section>
            </section>
