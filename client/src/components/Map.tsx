    ...
    useEffect(() => {
        navigator.geolocation.getCurrentPosition(async (position) => {
            ...
            try {
                ...
                const { countryName, countryCode, postcode, city } =
                    await response.json()
                setLocation({
                    longitude,
                    latitude,
                    ...
                    {isDisplayInfoWindow(selectedMarker) && (
                        ...
                        >
                            <div
                                ...
                                onClick={() =>
                                    navigate(`/gymclass/${selectedMarker.id}`)
                                }
                            >
                                <div className="w-full h-[50%] overflow-hidden rounded-lg">
                                    <img
                                        src={selectedMarker.postPic}
                                        alt={selectedMarker.exerciseName}
                                        className="w-full h-full object-cover"
                                    />
                                </div>
                                <h2 className="font-bold text-xl">
                                    {selectedMarker.exerciseName}
                                </h2>
                                <p className="text-sm">
                                    {selectedMarker.studioName}
                                </p>
                                <p className="text-sm">
                                    {formatDate(selectedMarker.classDate)}
                                </p>
                                <p className="text-sm">
                                    {formatTime(selectedMarker.classDate)}
                                </p>
                            </div>
                        </InfoWindow>
                    )}
